...
/**
 * Page Url Key generator
 */
class PageUrlKeyGenerator
{
    /**
     * @var PostFactory
     */
    ...
    public function generate(\Magento\Framework\DataObject $object, $field)
    {
        ...
        $urlKey = $object->getData($field) ? trim($object->getData($field)) : '';
        if (!$urlKey) {
            return;
        }
        ...
        $urlKey = str_replace($from, $to, $urlKey);
        ...
        $number = 1;
        while (true) {
            $finalUrlKey = $urlKey . ($number > 1 ? '-'.$number : '');

            $postId = $post->checkUrlKey($finalUrlKey, $object->getStoreId());
            $categoryId = $category->checkUrlKey($finalUrlKey, $object->getStoreId());
            ...
            if (!$postId && !$categoryId) {
                ...
                if ($postId
                    && $postId == $object->getId()
                    && $object instanceof \Victory\Blog\Model\Post
                ) {
                    break;
                }
                ...
                if ($categoryId
                    ...
                    && $object instanceof \Victory\Blog\Model\Category
                ) {
                    break;
                }
            }
        ...
        $object->setData('url_key', $finalUrlKey);
    }
}
